...
from .forms import PatientProfileForm  # Make sure this import is correct
...
def patient_profile(request):
    ...
    completed_tasks = PatientTask.objects.filter(patient=patient, status='completed')
    progress_data = get_weekly_progress(patient)
...
def send_sos_alert(request):
    if request.method == 'POST':
        ...
        SOSAlert.objects.create(
            patient=request.user,
            message=message,
            status='active'  # ✅ This must match your model's active status
        )
        return redirect('some_confirmation_page')
